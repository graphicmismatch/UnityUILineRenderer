...
public class UILineRenderer : Graphic {
    ...
    private void CreateLineSegment(Vector3 point1, Vector3 point2, VertexHelper vh) {
        ...
        vertex.position = point1Rotation * new Vector3(-thickness / 2, 0) + (point1 - offset);
        vh.AddVert(vertex);
        vertex.position = point1Rotation * new Vector3(thickness / 2, 0) + (point1 - offset);
        vh.AddVert(vertex);
        ...
        vertex.position = point2Rotation * new Vector3(-thickness / 2, 0) + (point2 - offset);
        vh.AddVert(vertex);
        vertex.position = point2Rotation * new Vector3(thickness / 2, 0) + (point2 - offset);
        vh.AddVert(vertex);
    ...
    private float RotatePointTowards(Vector2 vertex, Vector2 target) {
        return (float)(Mathf.Atan2(target.y - vertex.y, target.x - vertex.x) * Mathf.Rad2Deg);
    }
    /// <summary>
